...
const UPDATE_PERMISIIONS_MUTATION = gql`
...
class UserPermissions extends React.Component {
	...
	handlePermissionChange = (e) => {
		...
		// Figure out if we new to remove or add this permissions
		if (checkbox.checked) {
			// add it in!
	...
	render() {
		...
		return (
			<Mutation
				mutation={UPDATE_PERMISIIONS_MUTATION}
				variables={{
			...
			>
				{(updatePermissions, { loading, error }) => (
                    <>
                    {error && <tr><td colSpan="8"><Error error={error}></Error></td></tr>}
					<tr>
						<td>{user.name}</td>
						<td>{user.email}</td>
