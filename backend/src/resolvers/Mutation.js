...
const Mutations = {
	...
	async signin(parent, { email, password }, ctx, info) {
		// 1. Ckeck if there is a user with that email
		...
		const token = jwt.sign({ userId: user.id }, process.env.APP_SECRET);
		// 4. Set that cookie with the token
		ctx.response.cookie('token', token, {
			httpOnly: true,
			maxAge: 1000 * 60 * 60 * 24 * 365 // 1 year cookie
		});
	...
	async removeFromCart(parent, args, ctx, info) {
		...
		if(cartItem.user.id !== ctx.request.userId) {
			throw new Error('This Cart does not belongs you!');
		}
		// 3. Delete that card item
		return ctx.db.mutation.deleteCartItem({
			where: { id: args.id }
		}, info);
	},
	async createOrder(parent, args, ctx, info) {
		// 1. Query the current user and make sure they are sign in
		const { userId } = ctx.request;
		...
		console.log(`Go to charge for thw total of ${amount}`);
		...
		// 6. Clean up - clean the user cars, delete cartItems
